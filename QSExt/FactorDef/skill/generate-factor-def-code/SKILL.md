...
description: |
  生成符合 QSExt FactorDef 框架规范的因子定义 Python 脚本。根据用户需求或假设文档，
  查阅 FactorDef API 参考，验证数据可用性，产出可执行的 factor_def.py。
...
- 产出符合 FactorDef 框架规范的因子定义脚本（`factor_def.py`）
...
当需要以下任务时使用：
- 根据需求描述或假设文档生成因子定义 Python 脚本
- 为已有的因子逻辑编写符合 FactorDef 规范的代码
- 查阅 FactorDef API 参考（`__FACTOR_META__`、算子、自定义运算符等）
...
- **FactorDefInput 接口** — `fdi.FDB`、`fdi.Factors`、`fdi.ModelArgs` 等属性
- **defFactor 模式** — 标准模式、使用依赖因子、使用 ModelArgs
