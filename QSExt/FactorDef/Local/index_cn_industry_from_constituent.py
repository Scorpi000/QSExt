...
def defFactor(fdi: FactorDefInput, dep_fd: Dict[str, FactorDef]) -> FactorDef:
    ...
    calculateIndexIndustry = calcIndexIndustry.new(args={"DescriptorSection": [ComponentIDs, None, None]})
    
    for iTableName, iFactorName in fdi.ModelArgs["component_industry"]:
        FT = LDB.getTable(iTableName)
        iComponentIndustry = FT.getFactor(iFactorName)
        iFactor = calculateIndexIndustry(iComponentIndustry, ComponentID, ComponentWeight, factor_args={"Name": iFactorName})
