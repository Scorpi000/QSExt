...
def defFactor(fdi: FactorDefInput, dep_fd: Dict[str, FactorDef]) -> FactorDef:
    ...
    for _, iRow in WeightIndexList.iterrows():
        iIndexID = iRow["指数代码"]
        Factors.append(rename(JYDB.getTable("指数成份股权重", args={"AdditionalCondition": {"指数内部编码_R": str(iIndexID)}, "LookBack": 35, "OnlyLookBackDT": True}).getFactor("权重(%)") / 100, factor_name=iIndexID.replace(".", "_") + "_weight"))
