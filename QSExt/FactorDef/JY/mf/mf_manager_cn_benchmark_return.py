...
# 基金经理管理组合: 定期再平衡; 如果期间管理组合发生调整, 再平衡
def portfolio_return_d_fun(f, idt, iid, x, args):
    Return = pd.Series(x[1][-1, :], index=args["MFIDs"])
    Weight = pd.Series(x[2][0, :], index=args["MFIDs"])
    Weight[pd.isnull(Return)] = np.nan
    return np.fromiter((((Return.loc[iMID] * (Weight.loc[iMID] / Weight.loc[iMID].sum())).sum() if isinstance(iMID, list) else np.nan) for iMID in x[0][0, :]), dtype=float)

def portfolio_return(idx, idt, iid, x, args):
    pass

def portfolio_return_w_fun(f, idt, iid, x, args):
    Idx
