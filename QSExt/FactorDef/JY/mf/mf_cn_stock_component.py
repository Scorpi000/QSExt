...
def _uniqueID(z):
    ...
    return list(set(np.array(z)[pd.notnull(z)]))
...
def expandData(f, idt, iid, x, args):
    if not isinstance(x[1], list): return None
    if len(x[1])==0: return []
    return [x[0]] * len(x[1])
...
def aggrData(f, idt, iid, x, args):
    ...
    Rslt = pd.Series(x[1], index=x[0])
    return Rslt.groupby(axis=0, level=[0]).sum().loc[x[2]].tolist()
...
def genWeight(f, idt, iid, x, args):
    ...
    if np.nansum(Amt)>TotalAsset*(1+0.001):
        f.Logger.error(f"公募基金 '{iid}' 在报告期 '{idt}' 的股票持仓总金额为 {np.nansum(Amt)}, 超过了基金资产总值 {TotalAsset}, 将填充 NaN!")
        return [None] * Amt.shape[0]
...
def defFactor(args={}, debug=False):
    ...
    FT = JYDB.getTable("公募基金股票组合明细(公募基金ID)", args={"公告时点字段": None, "忽略时间": True, "投资类型": ""})
    ReportDate = FT.getFactor("报告期")
    InfoPublDate = FT.getFactor("信息发布日期")
    ComponentID = FT.getFactor("股票代码_R")
    PositionVolume = FT.getFactor("持股数量(股)")
    PositionAmount = FT.getFactor("市值(元)")
    PositionWeightInNV = FT.getFactor("占资产净值比例")
    
    FT = JYDB.getTable("公募基金重仓股票组合(公募基金ID)", args={"公告时点字段": None, "忽略时间": True, "投资类型": ""})
    KeyReportDate = FT.getFactor("报告期")
    KeyInfoPublDate = FT.getFactor("信息发布日期")
    KeyComponentID = FT.getFactor("股票内部代码_R")
    KeyPositionVolume = FT.getFactor("持股数量(股)")
    KeyPositionAmount = FT.getFactor("市值(元)")
    KeyPositionWeightInNV = FT.getFactor("占资产净值比例")    
    
    # 明细持仓缺失的使用重仓数据填充
    Mask = fd.notnull(ComponentID)
    ReportDate = fd.where(ReportDate, Mask, KeyReportDate, data_type="object")
    InfoPublDate = fd.where(InfoPublDate, Mask, KeyInfoPublDate, data_type="object")
    ComponentID = fd.where(ComponentID, Mask, KeyComponentID, data_type="object")
    PositionVolume = fd.where(PositionVolume, Mask, KeyPositionVolume, data_type="object")
    PositionAmount = fd.where(PositionAmount, Mask, KeyPositionAmount, data_type="object")
    PositionWeightInNV = fd.where(PositionWeightInNV, Mask, KeyPositionWeightInNV, data_type="object")
    PositionType = fd.where("重仓股", fd.notnull(KeyComponentID), None, data_type="string")
    PositionType = fd.where("全部持仓", Mask, PositionType, data_type="string")
    ...
    PositionType = fd.where(PositionType, Mask, None, data_type="string")
    ...
    PositionType = QS.FactorDB.PointOperation("持仓类型", [PositionType, UniComponentID], sys_args={"算子": expandData, "数据类型": "object"})
    ...
    PositionType = QS.FactorDB.SectionOperation("position_type", [PositionType, MainCode], sys_args={"算子": mapMainCode, "数据类型": "object"})
    
    Factors += [ReportDate, InfoPublDate, UniComponentID, PositionVolume, PositionAmount, PositionWeightInNV, PositionWeight, PositionType]
    
    UpdateArgs = {
        "因子表": "mf_cn_stock_component",
        "因子库参数": {"检查写入值": True},
