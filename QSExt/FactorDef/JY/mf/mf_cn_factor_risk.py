...
def defFactor(args={}, debug=False):
    ...
    for iLookBack, iLookBackPeriods in look_back_period.items():
        ...
        Factors.append(fd.fetch(max_drawdown, pos=0, dtype="double", factor_name=f"max_drawdown_rate_{iLookBack}"))
        Factors.append(fd.fetch(max_drawdown, pos=1, dtype="string", factor_name=f"max_drawdown_start_{iLookBack}"))
        Factors.append(fd.fetch(max_drawdown, pos=0, dtype="string", factor_name=f"max_drawdown_end_{iLookBack}"))

        # ####################### 最长回撤 #######################
        longest_drawdown = QS.FactorDB.TimeOperation(
            f"longest_drawdown_{iLookBack}",
            [FundReturn],
            sys_args={
        ...
        Factors.append(fd.fetch(longest_drawdown, pos=0, dtype="double", factor_name=f"longest_drawdown_{iLookBack}"))
        Factors.append(fd.fetch(longest_drawdown, pos=1, dtype="string", factor_name=f"max_drawdown_start_{iLookBack}"))
        Factors.append(fd.fetch(longest_drawdown, pos=0, dtype="string", factor_name=f"max_drawdown_end_{iLookBack}"))
        ...
        max_loss_ratio_relative_benchmark = QS.FactorDB.TimeOperation(
            f"max_loss_ratio_relative_benchmark_{iLookBack}",
            [FundReturn, BenchmarkReturn, Exist],
            sys_args={
