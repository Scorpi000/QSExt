...
def genWeight(f, idt, iid, x, args):
    ...
    if np.nansum(Amt)>TotalAsset*(1+0.001):
        f.Logger.error(f"公募基金 '{iid}' 在报告期 '{idt}' 的债券大类持仓总金额为 {np.nansum(Amt)}, 超过了基金资产总值 {TotalAsset}, 将填充 NaN!")
        return [None] * Amt.shape[0]
...
def defFactor(args={}, debug=False):
    ...
    FT = JYDB.getTable("公募基金债券组合结构", args={"公告时点字段": None, "忽略时间": True, "债券类型": ""})
    ReportDate = FT.getFactor("报告期")
    InfoPublDate = FT.getFactor("信息发布日期")
    ComponentID = FT.getFactor("债券类型")
    PositionAmount = FT.getFactor("市值(元)")
    PositionWeightInNV = FT.getFactor("占资产净值比例")
    PositionWeightInBond = FT.getFactor("占债券组合市值比例")
    ...
    PositionWeightInBond = fd.where(PositionWeightInBond, Mask, None, data_type="object")
    ...
    PositionWeightInBond = QS.FactorDB.PointOperation("债券持仓权重", [ComponentID, PositionWeightInBond, UniComponentID], sys_args={"算子": aggrData, "数据类型": "object"})
    ...
    PositionWeightInBond = QS.FactorDB.SectionOperation("weight_in_bond", [PositionWeightInBond, MainCode], sys_args={"算子": mapMainCode, "数据类型": "object"})
    
    Factors += [ReportDate, InfoPublDate, UniComponentID, PositionAmount, PositionWeightInNV, PositionWeight, PositionWeightInBond]
    
    UpdateArgs = {
        "因子表": "mf_cn_bond_type_component",
        "因子库参数": {"检查写入值": True},
