...
def defFactor(args={}, debug=False):
    ...
    look_back_period = {"3m": 13, "6m": 26, "1y": 52, "3y": 156, "5y": 260}# 回溯期
    min_period = 4# 最小期数
    min_period_ratio = 0.5# 最小期数比例
    FT_w = LDB.getTable("mf_cn_factor_risk_w")
    ...
    for time_node, window in look_back_period.items():
        ...
        beta = FT_w.getFactor(f"beta_{time_node}")
        max_drawdown_rate = FT.getFactor(f"max_drawdown_rate_{time_node}")
        down_volatility = FT_w.getFactor(f"down_volatility_{time_node}")
    ...
    UpdateArgs = {
        "因子表": "mf_cn_factor_risk_adjusted_w",
