...
def asset_decompose_fun(f, idt, iid, x, args):
    ...
    for i in range(Y.shape[1]):
        ...
        Prob = cvx.Problem(cvx.Minimize(cvx.norm2(iY - iX * beta)), [beta[1:]>=0, np.ones((1, iX.shape[1]-1)) * beta[1:]==1])
        try:
            Prob.solve()
        except:
            try:
...
if __name__ == "__main__":
    ...
    StartDT, EndDT = dt.datetime(2020, 10, 12), dt.datetime(2020, 10, 31)
