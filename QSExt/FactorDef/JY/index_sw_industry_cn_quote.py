# -*- coding: utf-8 -*-
"""申万行业指数行情"""
import datetime as dt

import datetime as dt
import pandas as pd

import QuantStudio.api as QS
fd = QS.FactorDB.FactorTools
Factorize = QS.FactorDB.Factorize

# args 应该包含的参数
# JYDB: 聚源因子库对象
# id_info_file: ID 配置信息文件地址
def defFactor(args={}, debug=False):
    Factors = []
    
    JYDB = args["JYDB"]
    
    FT = JYDB.getTable("申万指数行情")
    Factors.append(FT.getFactor("昨收盘(元-点)", new_name="pre_close"))
    Factors.append(FT.getFactor("今开盘(元-点)", new_name="open"))
    Factors.append(FT.getFactor("最高价(元-点)", new_name="high"))
    Factors.append(FT.getFactor("最低价(元-点)", new_name="low"))
    Factors.append(FT.getFactor("收盘价(元-点)", new_name="close"))
    Factors.append(FT.getFactor("成交量", new_name="volume"))
    Factors.append(FT.getFactor("成交金额(元)", new_name="amount"))
    Factors.append(FT.getFactor("成交笔数", new_name="turnover_deals"))
    Factors.append(FT.getFactor("指数市盈率", new_name="pe"))
    Factors.append(FT.getFactor("指数市净率", new_name="pb"))
    Factors.append(FT.getFactor("总市值(万元)", new_name="total_cap"))
    Factors.append(FT.getFactor("A股流通市值(万元)", new_name="float_cap"))
    Factors.append(Factorize(FT.getFactor("涨跌幅") / 100, factor_name="chg"))
    
    UpdateArgs = {
        "因子表": "index_sw_industry_cn_quote",
        "默认起始日": dt.datetime(2000, 1, 1),
        "最长回溯期": 365,
        "IDs": "外部指定"
    }
    
    return (Factors, UpdateArgs)

if __name__=="__main__":
    import logging
    Logger = logging.getLogger()
    
    JYDB = QS.FactorDB.JYDB(logger=Logger)
    JYDB.connect()
    
    #TDB = QS.FactorDB.SQLDB(config_file="SQLDBConfig_FactorTest.json", logger=Logger)
    TDB = QS.FactorDB.HDF5DB(logger=Logger)
    TDB.connect()
    
    Args = {"JYDB": JYDB, "id_info_file": [r"../conf/index/IndexIDs_cn.csv", r"../conf/index/IndexIDs_IndexMF.csv", r"../conf/index/IndexIDs_ETF.csv"]}
    Factors, UpdateArgs = defFactor(args=Args, debug=True)

    StartDT, EndDT = dt.datetime(2000, 1, 1), dt.datetime(2021, 6, 30)
    DTs = JYDB.getTradeDay(start_date=StartDT.date(), end_date=EndDT.date(), output_type="datetime")
    DTRuler = JYDB.getTradeDay(start_date=StartDT.date() - dt.timedelta(365), end_date=EndDT.date(), output_type="datetime")
    
    # 生成指数 ID
    IDs = []
    for iInfoFile in [r"../conf/sw_industry.csv", r"../conf/sw_industry_level2.csv"]:
        IDs += pd.read_csv(iInfoFile, index_col=None, header=None, encoding="utf-8", engine="python").iloc[:, 0].tolist()
    IDs = sorted(set(IDs))
    
    CFT = QS.FactorDB.CustomFT(UpdateArgs["因子表"])
    CFT.addFactors(factor_list=Factors)
    CFT.setDateTime(DTRuler)
    CFT.setID(IDs)

    TargetTable = CFT.Name
    CFT.write2FDB(factor_names=CFT.FactorNames, ids=IDs, dts=DTs,
                  factor_db=TDB, table_name=TargetTable,
                  if_exists="update", subprocess_num=0)

    TDB.disconnect()
    JYDB.disconnect()
