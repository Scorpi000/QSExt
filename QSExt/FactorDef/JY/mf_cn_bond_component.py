...
def genWeight(f, idt, iid, x, args):
    ...
    if np.nansum(Amt)>TotalAsset*(1+0.001):
        f.Logger.error(f"公募基金 '{iid}' 在报告期 '{idt}' 的债券持仓总金额为 {np.nansum(Amt)}, 超过了基金资产总值 {TotalAsset}, 将填充 NaN!")
        return [None] * Amt.shape[0]
...
def defFactor(args={}, debug=False):
    ...
    FT = JYDB.getTable("公募基金债券组合明细(公募基金ID)", args={"公告时点字段": None, "忽略时间": True, "报告期末是否处于转股期": "0"})
    ReportDate = FT.getFactor("报告期")
    InfoPublDate = FT.getFactor("信息发布日期")
    ComponentID = FT.getFactor("债券代码_R")
    PositionVolume = FT.getFactor("持股数量(张)")
