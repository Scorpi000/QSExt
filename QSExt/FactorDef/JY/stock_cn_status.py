...
def defFactor(args={}):
    ...
    ListDayNum = QS.FactorDB.PointOperation("listed_days", [ListDate],
                                            sys_args={"算子": ListDayNumFun, "运算时点": "多时点", "运算ID": "多ID"})
    Factors.append(ListDayNum)

    StatusChg = JYDB.getTable("上市状态更改").getFactor("变更类型", args={"回溯天数": np.inf})
    IfListed = QS.FactorDB.PointOperation("if_listed", [ListDate, StatusChg],
                                          sys_args={"算子": ifListed, "运算时点": "多时点", "运算ID": "多ID"})
