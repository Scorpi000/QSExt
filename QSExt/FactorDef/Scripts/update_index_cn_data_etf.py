...
from QSExt.FactorDef.JY import index_cn_info, index_cn_day_bar, index_cn_stock_component, index_cn_factor_value
from QSExt.FactorDef.Local import index_cn_factor_from_constituent
...
# 获取 ETF 对应的标的指数
def getETFIndex(JYDB, end_dt, org=None):
    ETFIDs = JYDB.getMutualFundID(type="ETF", is_current=False)

    if org is not None:
        FT = JYDB.getTable("公募基金概况")
        ETFInfo = FT.readData(factor_names=["基金类别", "设立日期", "存续期截止日", "上市日期", "基金管理人_R"], ids=ETFIDs, dts=[end_dt]).iloc[:, -1]
        ETFInfo = ETFInfo.rename(columns={"基金管理人_R": "基金管理人"})
        ETFInfo = ETFInfo[ETFInfo["基金管理人"]==org]
        ETFIDs = sorted(ETFInfo.index)

    FT = JYDB.getTable("公募基金ETF申购赎回清单信息", args={"LookBack": np.inf})
    ETFIndex = FT.readData(factor_names=["标的指数内部编码_R"], ids=ETFIDs, dts=[end_dt]).iloc[0, -1]
    return sorted(ETFIndex.dropna().unique())
...
def main(
    ...
    AStockIDs = SDB.getStockID(is_current=False)
    ETFIndexIDs = getETFIndex(JYDB=SDB, end_dt=EndDT, org="广发基金管理有限公司")# DEBUG
    if "index_etf_cn_info" in TDB.TableNames:
        FT = TDB.getTable("index_etf_cn_info")
        OldIDs = FT.getID()
        ExtraIDs = sorted(set(ETFIndexIDs).difference(OldIDs))
        ExtraDTs = FT.getDateTime()
    else:
        ExtraIDs, ExtraDTs = [], []

    ModelArgs = {"component_table": "index_cn_stock_component", "component_ids": AStockIDs}
    ModuleList = [
        (index_cn_info, {}, "index_etf_cn_info"),
        (index_cn_day_bar, {}, "index_etf_cn_day_bar"),
        (index_cn_stock_component, {}, "index_etf_cn_stock_component"),
    ...
    if ExtraIDs and ExtraDTs:
        ...
        for iModule, iModelArgs, iTargetTable in ModuleList:
            ...
            iStorer = FactorStorer(deps=iFactorDef.FactorList, args={"TargetFDB": TDB, "TargetTable": iTargetTable or iFactorDef.TargetTable, "IfExists": "update", "TableMeta": iTableMeta, "UpdateMeta": True})
        ...
        with FeatherFactorCache(args={"DTRuler": DTRuler, "PIDs": PIDList, "CacheDir": CacheDir, "StartMode": "new", "Suffix": ".pkl"}) as Cache:
            with FactorContext(Mode="DEBUG", PIDList=PIDList, DTRuler=DTRuler, SectionIDs=SectionIDs, DataCache=Cache) as Context:
                with (ParallelEngine() if workers > 0 else Engine()) as ExecEngine:
                    LocalContext = FactorLocalContext(DTs=ExtraDTs, IDs=IDs)
                    InitData = FactorInitData(DTRange=(ExtraDTs[0], ExtraDTs[-1]), SectionIDs=SectionIDs)
...
if __name__=="__main__":
    ...
    main(end_dt="2026-04-15", start_dt="2026-03-15", workers=0, cache_dir=r"C:\Users\hst\Project\Data\Cache\FactorTaskCache")