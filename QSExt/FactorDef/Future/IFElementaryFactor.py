...
Unit, Multiplier = FT.getFactor("交易单位(每手)"), FT.getFactor("合约乘数")
Factors.append(Factorize(Unit * Multiplier, factor_name="合约乘数"))
...
Factors.extend([PreSettle, FT.getFactor("开盘价(元)", new_name="开盘价"), FT.getFactor("最高价(元)", new_name="最高价"), 
                ...
                FT.getFactor("持仓量(手)", new_name="持仓量")])
...
if __name__=="__main__":
    ...
    Exchanges = ["CFFEX"]# 需要修改
    DefaultStartDT = dt.datetime(2010, 4, 16)# 需要修改
    TargetTable = "IFElementaryFactor"# 需要修改
    ...
    #IDs = ["IF1803.CFE", "IH1801.CFE", "IC1806.CFE"]# debug
    
    CalendarFT = WDB.getTable("中国期货交易日历")
    DTs = CalendarFT.getDateTime(iid="CFFEX", start_dt=StartDT, end_dt=EndDT)
    DTRuler = CalendarFT.getDateTime(iid="CFFEX", start_dt=StartDT-dt.timedelta(90), end_dt=EndDT)
