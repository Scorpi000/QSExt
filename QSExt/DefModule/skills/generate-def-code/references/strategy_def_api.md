# DefModule 策略定义 API 参考

本文档是 QSExt DefModule 策略定义框架的完整 API 参考，供生成策略定义脚本时查阅。

## 目录

1. [策略模块结构](#策略模块结构)
2. [__STRATEGY_META__ 字段详解](#__strategy_meta__-字段详解)
3. [DefInput 接口](#strategydefinput-接口)
...
与因子定义的 FactorDeps 完全一致，声明本策略依赖哪些因子：
...
`DefInput` 是 `defStrategy` 的唯一入参，包含运行时上下文。与因子定义的 DefInput 对齐，新增 `Strategies` 字段：
...
**不要导入的：**
- `Def`、`DefMeta` — 策略定义脚本不需要构造这些对象，框架自动包装
- `DefSettings`、`DefInputBuilder` — 运行时由执行脚本管理
- `build_dep_sd` — 框架内部使用
