...
将 ``Factor`` 对象列表生成为可执行的 DefModule 脚本（`.py` 文件），用于因子定义的导出、持久化和共享。
...
典型用法::
    ...
    generate_script(factors=[factor1, factor2], meta=meta,
                     output_path="~/DefModule/Scripts/my_factors.py")
"""
...
def generate_script(
    ...
    lines.append('')
    lines.append('')
    lines.append('def defNode(fdi: DefInput) -> List[Factor]:')
