...
class TickAccount(Account):
    ...
    def __init__(self, market_ft, name="Tick证券账户", sys_args={}, config_file=None, **kwargs):
        # 继承自 Account 的属性
        ...
        self._PositionNum = None# 持仓数量, array((nDT, nID))
        self._PositionAmount = None# 持仓金额, array((nDT, nID))
        self._Turnover = None# 换手率, array((nDT,))
        self._Orders = None# 当前接收到的订单, DataFrame(columns=["ID", "数量", "目标价"])
        self._LastPrice = None# 最新价, Series(index=self._IDs)
        self._TickData = None# Tick 数据, {}
