...
function MiningStudio() {
  ...
  useEffect(() => {
    import('../../services/connection').then(({ listConnections }) => {
      listConnections().then((data: any) => {
        const conns = (data || []).map((c: any) => ({ id: c.id || c.name, name: c.name }))
  ...
  const handleSubmit = useCallback(async () => {
    try {
      ...
      const config: GPRunConfig = {
        ...
        eval: {
          modules: values.eval_modules || [],
          transform: values.transform || 'abs',
          sign: values.sign || 'greater',
        },
      }
  ...
  }, [form, activeTask, isContinue, loadTasks])
          ...
          items={[
            {
              ...
              children: (
                ...
                >
                  ...
                  <Form.Item label="终端因子" name="terminal_factors">
                    <div>
                  ...
                  <Form.Item label="种子因子（可选）" name="seed_factors">
                    <Select
                      mode="multiple"
                      placeholder="从全局因子池选择种子因子，其叶节点自动作为终端因子"
                      ...
                      showSearch
                      notFoundContent={
                        <Empty description="因子池为空，请在右侧因子池面板中添加因子" image={Empty.PRESENTED_IMAGE_SIMPLE} />
                      }
                    />
                  </Form.Item>

                  <Form.Item label="导入因子定义脚本">
                    <Button
                      icon={<ImportOutlined />}
                  ...
                  <Form.Item label="初始深度范围" name="init_depth" />
                  ...
                  <Form.Item label="评估模块" name="eval_modules">
                    <Select
                      mode="multiple"
                      placeholder="选择评估模块"
                      options={
                        (frameworkConfig?.eval_modules || ['ic', 'multi_portfolio']).map((m: string) => ({
                          value: m,
                          label: m,
                        }))
                      }
                  ...
                  <Form.Item label="优化方向" name="sign">
                    <Select
                      options={[
                  ...
                  {runResult ? (
                    <>
                      ...
                      <div style={{ height: 300, background: '#fafafa', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {/* TODO: Plotly 图表 */}
                        <span style={{ color: '#999' }}>适应度进化曲线（Plotly 占位）</span>
                      </div>
                    </>
                  ) : (
