...
| `4-交易统计` | `交易概要` | 交易次数、持仓数等概要指标 |
| `4-交易统计` | `交易频率` | 按频率汇总的交易次数（频率由 `trade_freq` 配置控制） |
| `4-交易统计` | `交易金额` | 按频率汇总的交易金额 |
| `4-交易统计` | `换手率` | 按频率计算的换手率（交易金额 / 账户价值均值） |
...
modules:
  account_stats:
    enabled: true
  trade_freq: "M"   # 交易统计频率：D=日, W=周, M=月, Q=季, Y=年
```
