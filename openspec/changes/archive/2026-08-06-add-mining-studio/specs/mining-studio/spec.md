...
### Requirement: 挖掘框架列表
系统 SHALL 提供 API 返回可用的挖掘框架列表，包含每个框架的名称、描述和配置模板。

#### Scenario: 获取框架列表
- **WHEN** 前端请求 `GET /mining/frameworks`
- **THEN** 返回框架列表，当前至少包含 `gp`（遗传规划），每个条目包含 `key`, `name`, `description`, `config_schema`

#### Scenario: 获取框架配置模板
- **WHEN** 前端请求 `GET /mining/frameworks/{name}/config`
- **THEN** 返回该框架的完整配置模板，包括默认的 GP 参数、可用算子列表、评估模块配置模板

### Requirement: 挖掘任务提交
系统 SHALL 支持提交 GP 因子挖掘任务，每个任务是一个包含多次运行（run）的挖掘项目。首次运行配置包括终端因子选择、算子列表、GP 参数和适应度评估方案。

#### Scenario: 提交初始挖掘任务
- **WHEN** 用户在前端配置终端因子（从因子库选择）、算子、GP 参数（种群大小、代数等）和适应度评估方案后点击"开始挖掘"
- **THEN** 系统创建任务和第一个 run（run_001），返回 `task_id`，任务在后台异步执行，通过 WebSocket 推送进度
...
### Requirement: 任务进度与状态
系统 SHALL 复用 TaskManager 和 WebSocket 实时推送挖掘任务的状态和进度。
...
- **THEN** 系统删除该任务在 workspace 和任务管理器中���所有数据

### Requirement: 挖掘结果浏览
系统 SHALL 提供挖掘结果的查询，包括 Hall of Fame、适应度进化历史和因子树数据。支持按 run 切换查看，多个 run 的适应度曲线可拼接展示。
...
### Requirement: 前端页面
系统 SHALL 在 `/mining` 路由提供因子挖掘管理页面，包含左侧任务列表和右侧配置/结果面板。
...
#### Scenario: 新建任务并配置
- **WHEN** 用户在任务列表点击"新建任务"
- **THEN** 右侧显示配置面板，包含框架选择（GPFactor）、算子多选、终端因子搜索选择器、GP 参数字段（种群大小、代数、深度范围等）、评估模块选择
