...
`AiChatPanel` SHALL 在收到 `"init"` 消息后，将 `slash_commands` 列表存储在组件状态中，供 SlashCommandPalette 使用。

`slash_commands` SHALL 以 `string[]` 格式存储于 React state。

#### Scenario: 前端收到 init 消息并存储命令

- **WHEN** 前端通过 WebSocket 收到 `{"type":"init","data":{"slash_commands":["compact","clear","develop-factor"]}}`
- **THEN** AiChatPanel 的 `slashCommands` state 更新为 `["compact", "clear", "develop-factor"]`

#### Scenario: 未收到 init 消息时的默认行为

- **WHEN** AiChatPanel 挂载但尚未收到任何 `init` 消息
- **THEN** `slashCommands` state 为空数组 `[]`，SlashCommandPalette 不激活
