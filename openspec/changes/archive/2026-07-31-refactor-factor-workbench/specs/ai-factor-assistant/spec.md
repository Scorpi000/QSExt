...
默认使用 CLI 模式（`mode: "cli"`），可通过 `QSWebConfig.json` 切换为 SDK 模式（`mode: "sdk"`）。
...
- **THEN** `--add-dir` 参数确保 Claude 对 `scripts_dir` 和 `skill_dir` 有读写权限
...
- **THEN** 后端等待旧 CLI 进程自然退出（`session_persist_wait_sec` 秒），然后启动新 CLI 进程（`--resume <session_id>`），发送原始用户消息（不包装模板）

#### Scenario: 中断 Claude 操作（CLI）
- **WHEN** 前端发送 `{"action": "interrupt"}`
- **THEN** 后端调用 `process.terminate()` 终止 CLI 子进程

#### Scenario: 断开 Claude 连接（CLI）
- **WHEN** 前端发送 `{"action": "disconnect"}` 或 WebSocket 断开
- **THEN** 后端终止 CLI 子进程并清理资源
...
- **WHEN** 前端发送 `{"action": "start", "prompt": "..."}`
- **THEN** 后端创建 `AiService`，在 daemon 线程中运行 `ClaudeSDKClient.connect(prompt)`，配置 `ClaudeAgentOptions`（skills、mcp_servers、allowed_tools 等）
...
- **THEN** 前端自动调用 `POST /api/factors/import` 对生成的脚本执行 `importlib` 验证，展示元信息预览
