# factor-db-full-serialization

## Purpose

FactorDB 的 ConnectionJSON 存储从手动挑选字段迁移到 `__QS_Object__.serialize()` 完整输出，统一序列化协议并自动获得加密能力。配置文件路径不再存储，由 `__QS_Object__.__init__` 自动查找。
